...
func kubernetesRenderedServices(spec control.ProjectSpec) []kubernetesRenderedService {
	...
	services := []kubernetesRenderedService{
		db,
		kong,
		withRunAsNonRoot(kubernetesServiceFromControlSpec("meta", control.ServiceSpec{Enabled: true}, "supabase/postgres-meta:"+release.PostgresMeta, []kubernetesRenderedPort{{Name: "http", Port: 8080, TargetPort: 8080, Protocol: "TCP"}}, nil, nil, nil, metaKubernetesEnv(spec.Ref), enabledRenderedDependencies([]kubernetesRenderedDependency{{Service: "db", Port: 5432}}, states), httpProbe("/", 8080), httpProbe("/", 8080), []kubernetesRenderedWritable{{Name: "tmp", MountPath: "/tmp"}}), &upstreamImageRunAsNonRoot),
	}
	...
	auth := kubernetesServiceFromControlSpec("auth", authSpec, "supabase/gotrue:"+release.Auth, []kubernetesRenderedPort{{Name: "http", Port: 9999, TargetPort: 9999, Protocol: "TCP"}}, nil, nil, nil, authKubernetesEnv(), enabledRenderedDependencies([]kubernetesRenderedDependency{{Service: "db", Port: 5432}}, states), httpProbe("/health", 9999), httpProbe("/health", 9999), []kubernetesRenderedWritable{{Name: "tmp", MountPath: "/tmp"}})
	services = append(services, withRunAsNonRoot(auth, &upstreamImageRunAsNonRoot))
	known["auth"] = struct{}{}
	addKnown("rest", "postgrest/postgrest:"+release.REST, []kubernetesRenderedPort{{Name: "http", Port: 3000, TargetPort: 3000, Protocol: "TCP"}}, nil, nil, nil, []kubernetesRenderedDependency{{Service: "db", Port: 5432}}, httpProbe("/", 3000), httpProbe("/", 3000), nil)
	addKnown("graphql", "", nil, nil, nil, nil, nil, nil, nil, nil)
	addKnown("realtime", "supabase/realtime:"+release.Realtime, []kubernetesRenderedPort{{Name: "http", Port: 4000, TargetPort: 4000, Protocol: "TCP"}}, nil, nil, realtimeKubernetesEnv(spec.Ref), []kubernetesRenderedDependency{{Service: "db", Port: 5432}}, httpProbe("/", 4000), httpProbe("/", 4000), []kubernetesRenderedWritable{{Name: "tmp", MountPath: "/tmp"}})
	addKnown("storage", "supabase/storage-api:"+release.Storage, []kubernetesRenderedPort{{Name: "http", Port: 5000, TargetPort: 5000, Protocol: "TCP"}}, []kubernetesRenderedVolume{{Name: "data", MountPath: "/var/lib/storage", Size: "10Gi"}}, defaultIngress("storage-"+spec.Ref, domain, ""), storageKubernetesEnv(spec.Ref), []kubernetesRenderedDependency{{Service: "db", Port: 5432}, {Service: "rest", Port: 3000}}, httpProbe("/status", 5000), httpProbe("/status", 5000), []kubernetesRenderedWritable{{Name: "tmp", MountPath: "/tmp"}})
	addKnown("imgproxy", "darthsim/imgproxy:"+release.Imgproxy, []kubernetesRenderedPort{{Name: "http", Port: 5001, TargetPort: 5001, Protocol: "TCP"}}, []kubernetesRenderedVolume{{Name: "storage-data", MountPath: "/var/lib/storage", Size: "10Gi", Retain: true}}, nil, imgproxyKubernetesEnv(), nil, httpProbe("/", 5001), httpProbe("/", 5001), []kubernetesRenderedWritable{{Name: "tmp", MountPath: "/tmp"}})
	addKnown("functions", "supabase/edge-runtime:"+release.EdgeRuntime, []kubernetesRenderedPort{{Name: "http", Port: 9000, TargetPort: 9000, Protocol: "TCP"}}, nil, nil, functionsKubernetesEnv(spec.Ref), nil, httpProbe("/", 9000), httpProbe("/", 9000), []kubernetesRenderedWritable{{Name: "tmp", MountPath: "/tmp"}})
	addKnown("pooler", "supabase/supavisor:"+release.Pooler, []kubernetesRenderedPort{{Name: "transaction", Port: 6543, TargetPort: 6543, Protocol: "TCP"}, {Name: "session", Port: 5432, TargetPort: 5432, Protocol: "TCP"}, {Name: "http", Port: 4000, TargetPort: 4000, Protocol: "TCP"}}, nil, nil, poolerKubernetesEnv(spec.Ref), []kubernetesRenderedDependency{{Service: "db", Port: 5432}}, tcpProbe(6543), tcpProbe(6543), []kubernetesRenderedWritable{{Name: "tmp", MountPath: "/tmp"}})
	addKnown("studio", "supabase/studio:"+release.Studio, []kubernetesRenderedPort{{Name: "http", Port: 3000, TargetPort: 3000, Protocol: "TCP"}}, nil, defaultIngress("studio-"+spec.Ref, domain, ""), map[string]string{"HOSTNAME": "0.0.0.0"}, []kubernetesRenderedDependency{{Service: "meta", Port: 8080}, {Service: "kong", Port: 8000}}, httpProbe("/", 3000), httpProbe("/", 3000), []kubernetesRenderedWritable{{Name: "tmp", MountPath: "/tmp"}})
	addKnown("analytics", "supabase/logflare:"+release.Analytics, []kubernetesRenderedPort{{Name: "http", Port: 4000, TargetPort: 4000, Protocol: "TCP"}}, nil, nil, analyticsKubernetesEnv(spec.Ref), []kubernetesRenderedDependency{{Service: "db", Port: 5432}}, httpProbe("/", 4000), httpProbe("/", 4000), []kubernetesRenderedWritable{{Name: "tmp", MountPath: "/tmp"}})
	addKnown("vector", "timberio/vector:"+release.Vector, nil, nil, nil, nil, nil, nil, nil, []kubernetesRenderedWritable{{Name: "tmp", MountPath: "/tmp"}})
