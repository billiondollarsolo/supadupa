...
export function ProjectFunctionsPage() {
  const { activeFeatureFlags, activeProject, functionRegions, functionStorageMounts, projectConfig, projectFunctions, projectLogs } = useDashboardContext();
  ...
  return (
    <ProjectPage>
      <FunctionsPanel
        ...
        functions={projectFunctions.data ?? []}
        regions={functionRegions.data ?? []}
        mounts={functionStorageMounts.data ?? []}
        logs={projectLogs.data ?? []}
        loading={projectFunctions.isLoading || functionRegions.isLoading || functionStorageMounts.isLoading}
        enabled={enabled}
        item={showingFunctionDetail ? item : undefined}
      />
