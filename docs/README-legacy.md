...
- `docs/` - product, architecture, and compatibility documentation, including the Supabase compatibility suite in `docs/supabase-compat-test-suite.md`.
...
The service listens on `:8080` by default. Override with `SUPADUPA_ADDR`. Native dev allows common local admin origins (`localhost`/`127.0.0.1` on `3000`, `3001`, `5173`, and `5174`) when `SUPADUPA_CORS_ORIGINS` is unset. The deployed Compose stack defaults CORS to only `https://$SUPADUPA_ADMIN_HOST`; add local origins to `SUPADUPA_CORS_ORIGINS` explicitly only for trusted development access. Deployed admin origins are also derived from `SUPADUPA_ADMIN_HOST` or `SUPADUPA_ADMIN_URL` when the variable is left unset. Set `SUPADUPA_AUTH_SECRET` to a stable high-entropy value for bearer-token signing; when it is omitted, supadupa falls back to `SUPADUPA_SECRET_KEY` before using the local-development signing key.
...
- `studio-<ref>.projects.example.test` is covered by `*.projects.example.test`
...
Custom domain creates also prepare certificate state under `./runtime/certs/<ref>`. Without an operator command, supadupa writes a manual JSON plan and leaves `cert_status=pending`/`cert_mode=manual` while Traefik handles ACME from the rendered TLS router. Configure `SUPADUPA_CERT_COMMAND` to automate issuance; stdout/stderr becomes a certificate transcript and successful runs mark the domain `issued`. Template variables include `{{ref}}`, `{{project_ref}}`, `{{fqdn}}`, `{{cert_path}}`, and `{{cert_root}}`. Project admins can also upload a BYO certificate/key pair from the domain detail page or `PUT /v1/projects/{ref}/domains/{fqdn}/certificate`; the private key is written to `runtime/certs/<ref>/<fqdn>.key`, never returned by the API, and Traefik reads it through the edge-router `/certs` mount. Use `DELETE /v1/projects/{ref}/domains/{fqdn}/certificate` to remove the uploaded keypair and return the domain to managed/manual ACME flow. Custom domains cannot claim platform hosts, project base domains, or generated project hosts such as `<ref>.<apps-domain>`, `studio-<ref>.<apps-domain>`, `db-<ref>.<apps-domain>`, or `pooler-<ref>.<apps-domain>`; in the common `*.example.com` control plane plus `*.apps.example.com` project topology, `admin.example.com` and `api.example.com` are reserved too.
...
When `SUPADUPA_COMPOSE_APPLY=true`, project reconciliation uses live Docker Compose state in addition to rendered desired state. `Status` runs `docker compose ps --format json` for each project stack and marks projects degraded when required containers are missing, exited, or unhealthy. Project detail responses include `runtime_status.services`, a structured per-service breakdown of desired state, Compose service name, container state, health, exit code, and operator-facing message. When apply mode is false, status remains a dry-run render/drift check so local development can validate generated stacks without a Docker daemon.
...
Project config updates for auth, auth providers, email templates, storage, functions, realtime, pooler, database markers, and SMTP are synchronized into each Compose project's `.env` when they map to upstream runtime settings. Network config updates re-render Traefik route policy. On control-plane startup, Supadupa replays persisted project secrets, service toggles, explicit runtime configs, Auth Hooks, and replicas through the provisioner so existing projects pick up generator fixes after upgrades. Config values that do not have a direct stack runtime setting remain control-plane metadata for UI/API parity. Provider client secrets, captcha secrets, SMS tokens, and SMTP passwords are represented as secret handles in config; write the material with `PUT /v1/projects/<ref>/secrets/<kind>`, delete obsolete custom material with `DELETE /v1/projects/<ref>/secrets/<kind>`, and reference it as `secret://projects/<ref>/<kind>`. During runtime config sync Supadupa resolves those handles into the concrete GoTrue/SMS/SMTP env values that upstream services require, while API/UI responses keep handles and masked secret metadata. Unresolved or cross-project handles return a conflict and roll back the visible config update.
...
Phone login and phone MFA SMS config supports GoTrue-compatible providers `twilio`, `twilio_verify`, `messagebird`, `textlocal`, and `vonage`. Provider credential values use `secret://` handles for tokens, access keys, and API secrets; Compose resolves revealable project handles and writes the matching `GOTRUE_SMS_*` runtime settings.

Social OAuth config supports the hosted-style named provider surface through project config area `auth_providers`: Apple, Bitbucket, Discord, Facebook, GitLab, GitHub, Google, Kakao, Keycloak, LinkedIn OIDC, Notion, Slack OIDC, Spotify, Twitch, Twitter, WorkOS, Zoom, Azure, and custom OIDC metadata. Provider client secrets use `secret://` handles; Compose resolves revealable project handles and writes matching `GOTRUE_EXTERNAL_*` runtime settings for named GoTrue providers while retaining `SUPADUPA_AUTH_OIDC_*` metadata for custom OIDC.
...
Auth Hooks are declared through `/v1/projects/<ref>/auth/hooks`. Hook types such as `custom_access_token`, `before_user_created`, `send_sms`, and `send_email` can target an HTTPS endpoint or a project Edge Function. Compose synchronizes enabled hooks into GoTrue `GOTRUE_HOOK_*_ENABLED`, `GOTRUE_HOOK_*_URI`, and resolved Standard Webhooks `GOTRUE_HOOK_*_SECRETS` settings, maps Edge Function targets to the project's `/functions/v1/<name>` URL, and writes the complete handle-backed desired state to each project `auth-hooks.json` artifact. Kubernetes writes the same hook set to an `auth-hooks.yaml` `ProjectAuthHooks` manifest for the operator to reconcile atomically. Secret handles and sensitive headers are masked in API/UI responses and retained as `SUPADUPA_AUTH_HOOK_*_SECRET_HANDLE` metadata; unresolved or cross-project handles return a conflict and roll back the hook metadata.
...
Platform SCIM v2 provisioning is available under `/v1/scim/v2` for Enterprise-style IdP provisioning. Dashboard admins can inspect SCIM state with their normal platform session, while external IdPs must use the write-only SCIM bearer token configured in `/v1/settings/sso`; only a token hash is stored, API responses expose `scim_token_configured`, and audit metadata never includes token material. Users map to platform users, and groups map to org teams; group create uses `externalId` or the `urn:supadupa:params:scim:schemas:extension:Group.org_id` extension to select the target org. SCIM user deprovisioning through `DELETE /Users/{id}` or `PATCH active:false` removes the platform user, org memberships, team memberships, and user-level project grants, then records the action in the immutable audit chain.
...
Platform SCIM provisioning is exposed through `/v1/scim/v2/ServiceProviderConfig`, `/v1/scim/v2/Users`, and `/v1/scim/v2/Groups`. The admin Settings screen shows whether SCIM is enabled, whether a token is configured, service-provider capabilities, and provisioned user/group summaries so operators can verify IdP provisioning state without leaving the dashboard.
...
Edge Function runtime config includes `worker_timeout_ms`, defaulting to `60000`. The Compose provisioner syncs it to `FUNCTION_WORKER_TIMEOUT_MS`, and the generated dispatcher returns HTTP 504 with the configured timeout when a function request exceeds that limit. Existing projects inherit new function-config defaults on read/update so control-plane upgrades do not leave saved partial configs missing newly supported runtime settings.
...
Persistent storage mounts for Edge Functions are declared through `/v1/projects/<ref>/functions/storage-mounts`. Each declaration links a deployed function to a project storage bucket, desired mount path under `/mnt`, optional bucket prefix, read-only/read-write mode, and environment alias. Compose mounts the project Storage volume into Edge Runtime read-only, writes each function's `storage-mounts.json` artifact, and the generated dispatcher materializes a POSIX-style read view inside the worker service tree before invoking the worker. The environment alias resolves to the runtime-readable mounted object view; substrates that support arbitrary mounts can honor the declared `/mnt` path directly. Functions should still write objects through the Storage API so bucket metadata, versioning, and policies stay consistent.

Project storage buckets are managed through `/v1/projects/<ref>/storage/buckets`. In runtime apply mode (`SUPADUPA_COMPOSE_APPLY=true`, `SUPADUPA_K8S_APPLY=true`, or explicit `SUPADUPA_STORAGE_APPLY=true`), bucket create/delete calls the project's live Supabase Storage API with the project service-role key, so the Management API state and Storage data plane stay aligned. If live bucket creation fails, Supadupa rolls back the metadata record and returns a conflict instead of presenting a bucket that does not exist in Storage. Set `SUPADUPA_STORAGE_APPLY=false` to keep this surface metadata-only for render/test environments, or `SUPADUPA_STORAGE_APPLY_BASE_URL` for nonstandard local routing.

Read replicas are provisioned through `/v1/projects/<ref>/replicas`. Each replica records resource tier, region, read-routing weight, failover priority, lag metadata, and role. `/v1/projects/<ref>/replicas/routing` returns the weighted healthy read target set and next failover candidate; `/promote` and `/failover` update the authoritative control-plane routing state and emit logs/audit events. Replica responses expose a remote-safe `public_read_uri` and `read_uri` at `db-replica-<replica>-<ref>.<apps-domain>:5432?sslmode=require`, while `internal_read_uri` remains the Docker-network runtime address. Compose reconciles a deterministic `replicas/compose.yaml` overlay, writes per-replica env metadata, opens primary replication access for the Docker network, joins replicas to the shared ingress network with stable aliases for Traefik TCP routing, bootstraps standby services with `pg_basebackup`, and in applied mode reloads the primary through the generated `supabase_admin` superuser before verifying each replica reports `pg_is_in_recovery()`. Cold replica boot tolerance defaults to 240 seconds and can be changed with `SUPADUPA_REPLICA_READY_TIMEOUT_SECONDS`. Project destroy includes the replica overlay so replica services and volumes are removed with the rest of the project unless volumes are intentionally retained. Kubernetes writes the same desired state to the `ProjectReplica` CRD for an operator to enact streaming replication, read routing, and promotion.
...
Database extensions are listed and toggled through `/v1/projects/<ref>/database/extensions`. The API starts from the upstream Compose init catalog (`pgcrypto`, `uuid-ossp`, `pg_graphql`, `pg_stat_statements`, `pg_cron`, `pgmq`, `vector`, and `supabase_vault`) and records per-project overrides for enabled state, schema, and pinned version. In Compose runtime apply mode, extension updates execute live Postgres DDL with `CREATE SCHEMA IF NOT EXISTS`, `CREATE EXTENSION IF NOT EXISTS`, or `DROP EXTENSION IF EXISTS`; failed DDL rolls the visible extension state back to the previous control-plane value.

Database cron jobs are declared through `/v1/projects/<ref>/database/cron-jobs`. The control plane records pg_cron schedules, SQL commands, database/user targets, active state, runtime limits, masked metadata, project logs, audit events, and usage metrics. In Compose runtime apply mode, active cron declarations call live `cron.schedule_in_database`, deletes call `cron.unschedule` by the resolved job id, and failed live scheduling rolls back the metadata record.

Database queues are declared through `/v1/projects/<ref>/database/queues`. The control plane records pgmq queue names, schema, retention window, visibility timeout, retry/dead-letter policy, active state, masked metadata, project logs, audit events, and usage metrics. In Compose runtime apply mode, active queue declarations create the pgmq schema/extension when needed, call live `pgmq.create` for the queue and optional dead-letter queue, and deletes call `pgmq.drop_queue`; failed live create rolls back the metadata record.

Database webhooks are declared through `/v1/projects/<ref>/database/webhooks`. The control plane records trigger schema/table, insert/update/delete events, HTTPS delivery endpoint, method, headers, timeout/retry policy, active state, masked metadata, project logs, audit events, and usage metrics. In Compose runtime apply mode, active POST webhooks create the live `pg_net` extension when needed, render a `supadupa` schema trigger function, create one `AFTER` trigger per selected event, and delete those triggers/functions before removing metadata. Sensitive delivery headers must reference revealable project secret handles; Supadupa resolves them into the trigger function and never writes `secret://` handles into live DDL. Failed live DDL or unrevealable headers roll back the metadata record.

Declarative database schemas are recorded through `/v1/projects/<ref>/database/schemas`. The control plane stores migration name, version, target schema, SQL text, SHA-256 checksum, apply order, active state, masked metadata, project logs, audit events, and usage metrics. In Compose runtime apply mode, active schema declarations are executed against the live project Postgres through `psql`; failed SQL rolls back the control-plane metadata record so the UI does not report unapplied DDL as configured. Inactive declarations remain metadata-only migration records.

Database roles are declared through `/v1/projects/<ref>/database/roles`. The control plane records role flags, connection limits, schema grants, memberships, masked password secret handles, project logs, audit events, and usage metrics. Reserved upstream roles such as `anon`, `authenticated`, `service_role`, and `supabase_admin` cannot be replaced through this surface. In Compose runtime apply mode, role creates and deletes execute live Postgres DDL for login/no-login flags, inheritance, BYPASSRLS, connection limits, role memberships, and schema/table grants. Login roles only accept revealable `secret://projects/<ref>/<kind>` handles for the role password; failed live DDL or unrevealable handles roll back the metadata record so the UI does not report unapplied roles as configured.
...
Branch creates provision a full isolated branch project and default to a schema-only/data-less branch, matching hosted Supabase preview branch behavior. Set `with_data: true` in the API, `--with-data` in the CLI, or the Terraform `with_data` attribute to request a source data clone. Compose writes requested clone artifacts under `./runtime/projects/<branch-ref>/branch-clone`; without a live clone command, supadupa writes an explicit dry-run `clone-plan.sql`. Configure `SUPADUPA_BRANCH_CLONE_COMMAND` to run a real source-to-branch dump/restore; stdout/stderr becomes the clone transcript. Template variables include `{{source_ref}}`, `{{branch_ref}}`, `{{branch_id}}`, `{{branch_name}}`, `{{expires_at}}`, `{{source_dir}}`, `{{branch_dir}}`, and `{{clone_path}}`. Kubernetes writes a `branch-clone.yaml` `ProjectBranchClone` manifest beside the branch `Project` resource when data cloning is requested so an operator can reconcile source-to-branch data copy in-cluster.

Log drain creates write Vector sink fragments under `./runtime/projects/<ref>/log-drains/<drain-id>.toml` by default. Compose mounts that directory into Vector at `/etc/vector/log-drains` alongside the base per-project `vector.yml`. The base Compose Vector config ingests Docker logs through the local Docker socket and filters by the project's `com.docker.compose.project=<ref>` label before fan-out to configured drains.

Logical backups are written under `./runtime/backups` by default. In applied Compose mode (`SUPADUPA_COMPOSE_APPLY=true`), supadupa defaults to a real logical backup command that runs `docker compose -p {{ref}} -f {{compose_file}} exec -T db pg_dump --clean --if-exists --quote-all-identifiers --schema=public --schema=auth --schema=storage --schema=supabase_migrations -U postgres postgres`; stdout becomes the backup artifact. Set `SUPADUPA_COMPOSE_BACKUP_DEFAULTS=false` to disable those Compose defaults, or configure `SUPADUPA_LOGICAL_BACKUP_COMMAND` to supply a custom command. Without an applied Compose default or custom command, supadupa writes an explicit dry-run `.sql` artifact for local development. Every backup records size, SHA-256 checksum, and `verified_at` after the artifact is readable. Scheduled backup successes and failures are both recorded in project logs and the immutable audit chain. The control-plane scheduler checks due backup policies every minute by default; set `SUPADUPA_BACKUP_SCHEDULER_TICK` to a Go duration such as `30s` or `5m` to tune that loop. Template variables are shell-quoted and include `{{ref}}`, `{{project_ref}}`, `{{project_id}}`, `{{stack_version}}`, `{{project_dir}}`, `{{compose_file}}`, `{{backup_root}}`, and `{{backup_path}}`.

Platform admins can register one or more S3-compatible backup targets from Admin UI -> Settings -> Backups or through `/v1/backup-storage-targets`. Target credentials are accepted on create/update and redacted from API responses; the encrypted control-plane checkpoint stores the secret material. Use `POST /v1/backup-storage-targets/<id>/test` or the Settings -> Backups test button to prove the target can write and read a small probe object before relying on it for recovery. A project backup policy can select a target, or use the platform default target when one is marked default. Policies support `logical` and `physical` kinds. When a target is selected, manual and scheduled project backups upload a copy after the local artifact is verified and record `remote_location` as `s3://bucket/key`; pre-upgrade guardrail backups remain logical so the existing SQL restore path can verify them. Targets whose endpoint resolves to loopback, an unspecified address, `host.docker.internal`, or an IP assigned to the same machine do not satisfy off-host recoverability. `/v1/projects/<ref>/recoverability` derives the current recovery posture from backup policy, verified backup artifacts, off-host target state, PITR policy, and WAL archives; the Admin UI Backups page uses it to distinguish local-only backups, verified off-host backups, and restore-to-time readiness. The Compose logical default backs up application data schemas (`public`, `auth`, `storage`, and `supabase_migrations`) and leaves full internal-stack recovery to physical backup policy support.

The default S3-compatible target can also be bootstrapped from `.env` on control-plane startup. Set `SUPADUPA_BACKUP_TARGET_BUCKET`, `SUPADUPA_BACKUP_TARGET_ACCESS_KEY_ID`, and `SUPADUPA_BACKUP_TARGET_SECRET_ACCESS_KEY`; optionally set `SUPADUPA_BACKUP_TARGET_NAME`, `SUPADUPA_BACKUP_TARGET_ENDPOINT`, `SUPADUPA_BACKUP_TARGET_REGION`, `SUPADUPA_BACKUP_TARGET_PREFIX`, and `SUPADUPA_BACKUP_TARGET_FORCE_PATH_STYLE`. The `SUPADUPA_BACKUP_S3_*` aliases are accepted for the same fields. Startup creates or updates the named target, marks it as the platform default, preserves an existing secret if the env update omits the secret, and skips changes when the stored target already matches. Set `SUPADUPA_BACKUP_TARGET_AUTO_TEST=true` to run the server-side target probe during startup and record `last_test_*` without a manual UI step. For hosted-grade production recovery, also set `SUPADUPA_REQUIRE_RECOVERY_READY_TARGETS=true` and `SUPADUPA_REQUIRE_DURABLE_UPGRADE_BACKUP=true` so physical backups, WAL uploads, PITR bucket derivation, and upgrades require a tested durable off-host target. For Cloudflare R2, set the endpoint to the account R2 S3 endpoint, region to `auto`, and provide the R2 access key credentials.
`GET /v1/runtime-config` returns admin-only, redacted operational booleans for the running control-plane process, including Compose/Kubernetes apply mode, backup command availability, dry-run state, and the recovery/upgrade guard flags. It intentionally does not return command strings or credentials. The compatibility runner uses this endpoint to fail hosted-grade recovery and upgrade profiles when the test runner requested strict guards but the remote API process was not restarted with those guards enabled.

The CLI exposes the same target workflow for automation:

```bash
go run ./cmd/supadupa-cli backup-targets create \
...
go run ./cmd/supadupa-cli backup-targets test --id <target-id>
go run ./cmd/supadupa-cli backups set-policy --ref smoke --enabled --schedule hourly --kind physical --storage-target-id <target-id>
```

Supadupa control-plane backups are managed from the same Settings -> Backups surface or through `/v1/platform/backups`. A control-plane backup writes a local `supadupa-control-plane-backup/v1` artifact under `./runtime/backups/_platform`. When the control plane is using the persistent meta DB, that artifact contains the encrypted checkpoint payload rather than plaintext project metadata or secrets. If a default S3-compatible target is configured, the backup is copied off-host under `s3://<bucket>/<prefix>/platform/backups/...`. Platform backups can be restored from Settings -> Backups or with `POST /v1/platform/backups/<id>/restore`; both paths require the exact `restore-control-plane` confirmation. Restore validates artifact size and checksum, can rehydrate a missing local artifact from the recorded S3-compatible target, imports checkpoint and normalized metadata atomically, preserves the source backup record, then asks the provisioner to reconcile restored projects and stop projects removed by the checkpoint while retaining their volumes. The API returns `restore_state=reconciled` when runtime reconciliation succeeds, or `metadata-restored-runtime-errors` with runtime errors when provisioner work needs operator follow-up.

Project upgrades are limited to stable stack versions from `SUPADUPA_SUPPORTED_STACK_VERSIONS` or the built-in stable set. Each stable version resolves through a stack release manifest that maps every Compose image tag, including Postgres, Kong, Studio, postgres-meta, GoTrue, PostgREST, Realtime, Storage, Edge Runtime, Supavisor, Logflare, Vector, and imgproxy. Override or extend release manifests with `SUPADUPA_STACK_RELEASES_JSON`, either as a JSON object keyed by version or a list of manifest objects with a `version` field. Configured supported versions that do not have a built-in or explicit manifest are not exposed as upgrade targets; Supadupa does not create Postgres-only pseudo releases. `GET /v1/stack-releases` returns the active manifests, and the admin UI uses that endpoint for project upgrade choices. The upgrade API creates or verifies a completed logical pre-upgrade backup before applying the target stack, records previous/target version metadata, and attempts to roll the runtime back to the previous version if the provisioner upgrade fails. Failed upgrade responses include the error, backup metadata, previous/target versions, rollback availability, rollback attempt state, and any rollback error so the UI/API caller can immediately restore from the pre-upgrade backup if needed. Set `SUPADUPA_UPGRADE_FAILURE_AUTO_RESTORE=true` only when failed-upgrade rollback should also run the logical restore command against the pre-upgrade backup; responses then report `restore_attempted`, `restore_state`, and `restore_error`. A supplied `backup_id` must be completed, verified, readable or rehydratable from its recorded S3-compatible target, size-matched, and checksum-matched before it can be used for upgrade. Set `SUPADUPA_REQUIRE_DURABLE_UPGRADE_BACKUP=true` in production to reject upgrades unless the selected pre-upgrade backup also has a remote artifact on a tested durable off-host S3-compatible target. The pre-upgrade backup remains available through the restore workflow. Compatibility tests can enable guarded failure injection with `SUPADUPA_COMPAT_UPGRADE_FAILURE_TARGETS` plus the `X-Supadupa-Compat-Inject-Upgrade-Failure: true` header; leave that environment unset outside disposable validation runs.

Example custom backup command for a running Compose project:

```bash
export SUPADUPA_LOGICAL_BACKUP_COMMAND='docker compose -p {{ref}} -f {{compose_file}} exec -T db pg_dump --clean --if-exists --quote-all-identifiers -U postgres postgres'
```

Restores also use a real Compose default in applied Compose mode unless `SUPADUPA_COMPOSE_BACKUP_DEFAULTS=false` is set: the command reads `POSTGRES_PASSWORD` from the project `.env` and runs `docker compose -p {{ref}} -f {{compose_file}} exec -T -e PGPASSWORD="$db_password" db psql -v ON_ERROR_STOP=1 -U supabase_admin postgres < {{backup_path}}`. Configure `SUPADUPA_LOGICAL_RESTORE_COMMAND` to supply a custom restore command; stdout/stderr becomes the restore transcript. Without an applied Compose default or custom command, restores write an explicit dry-run `.sql` restore plan. Template variables include `{{ref}}`, `{{project_ref}}`, `{{project_dir}}`, `{{compose_file}}`, `{{backup_root}}`, `{{backup_id}}`, `{{backup_kind}}`, `{{backup_path}}`, `{{restore_root}}`, and `{{restore_path}}`.

Physical backup policies use a real Compose default in applied Compose mode unless `SUPADUPA_COMPOSE_BACKUP_DEFAULTS=false` is set: Supadupa runs `pg_basebackup` inside the project `db` container, packages the generated base-backup files as a verified `physical` artifact, and uploads it to the selected S3-compatible target when configured. Outside applied Compose mode, configure `SUPADUPA_PHYSICAL_BACKUP_COMMAND`; Supadupa does not create dry-run physical backups because they would falsely imply PITR readiness. Use this hook for WAL-G, pgBackRest, `pg_basebackup`, or a substrate operator wrapper. Template variables match logical backups: `{{ref}}`, `{{project_ref}}`, `{{project_id}}`, `{{stack_version}}`, `{{project_dir}}`, `{{compose_file}}`, `{{backup_root}}`, and `{{backup_path}}`.

WAL archives use a real Compose default in applied Compose mode unless `SUPADUPA_COMPOSE_BACKUP_DEFAULTS=false` is set: Supadupa switches WAL inside the project `db` container and streams the completed WAL segment into the archive artifact. Outside applied Compose mode, configure `SUPADUPA_WAL_ARCHIVE_COMMAND` to run WAL-G, pgBackRest, or an operator wrapper; stdout becomes the WAL artifact. Without a Compose default or explicit command, WAL archive requests fail instead of creating fake artifacts that could falsely imply PITR readiness. `SUPADUPA_WAL_ARCHIVE_DRY_RUN=true` remains available for local tests only. Each archive records size, SHA-256 checksum, and `verified_at` after the artifact is readable. When PITR is enabled for a healthy or degraded project, the control-plane scheduler archives WAL automatically every 5 minutes by default; set `SUPADUPA_WAL_ARCHIVE_INTERVAL` to a Go duration such as `2m` or `15m` to tune that cadence. If PITR is enabled without an explicit archive bucket, Supadupa derives `s3://<bucket>/<prefix>/projects/<ref>/wal` from the selected project backup target or platform default target. Manual archive actions use the same archive path and audit records. Template variables include `{{ref}}`, `{{project_ref}}`, `{{project_id}}`, `{{domain}}`, `{{project_dir}}`, `{{compose_file}}`, `{{backup_root}}`, `{{segment}}`, `{{archive_bucket}}`, `{{retention_days}}`, `{{wal_root}}`, and `{{wal_path}}`.

Point-in-time restores use the hosted Supabase Management API shape: `GET /v1/projects/<ref>/database/backups` lists backups and `POST /v1/projects/<ref>/database/backups/restore-pitr` accepts `{"recovery_time_target_unix":"<unix-seconds>","confirmation":"restore pitr project <ref>"}`. The restore endpoint returns HTTP 201 after a successful restore command and refuses to start until recoverability proves PITR is enabled, the selected S3-compatible target has passed a server-side validation probe, a verified physical off-host base backup exists, verified WAL archives define the recovery window, the requested timestamp is inside that window, and a restore command is configured. In applied Compose mode, Supadupa provides a default destructive restore command that stops the project stack, including `replicas/compose.yaml` when read replicas exist, restores the `db-data` volume from the verified physical backup, copies the selected WAL range into `pg_wal`, writes recovery target settings, and starts the stack again with the same compose-file set. Outside applied Compose mode, configure `SUPADUPA_PITR_RESTORE_COMMAND` for WAL-G, pgBackRest, or substrate-specific replay. Before running the restore command, Supadupa validates that the selected physical backup artifact and every verified off-host WAL artifact needed from that base backup through the requested recovery target are readable, size-matched, and checksum-matched. The command stdout/stderr becomes the restore transcript. Template variables include `{{ref}}`, `{{project_ref}}`, `{{project_id}}`, `{{domain}}`, `{{project_dir}}`, `{{compose_file}}`, `{{compose_file_args}}`, `{{compose_files}}`, `{{backup_root}}`, `{{backup_id}}`, `{{backup_kind}}`, `{{backup_path}}`, `{{backup_remote_location}}`, `{{wal_archive_id}}`, `{{wal_archive_ids}}`, `{{wal_segment}}`, `{{wal_segments}}`, `{{wal_path}}`, `{{wal_path_args}}`, `{{wal_paths}}`, `{{restore_root}}`, `{{restore_path}}`, `{{recovery_time_target}}`, and `{{recovery_time_target_unix}}`. Full hosted-grade recovery still requires a durable off-host target and destructive disposable restore validation for the selected substrate.
...
For hosted Supabase-style project binding, use `projects link --ref <ref> --dir <workspace>`. It writes `.supadupa/project.json`, `.supadupa/config.toml`, `.supadupa/supabase.env`, and a conventional `supabase/config.toml` when that file is missing so official Supabase CLI database commands can run from the linked workspace without relying on Supabase Cloud `supabase link`. Existing `supabase/config.toml` files are preserved. The default env file contains secret handles. Use `projects env --ref <ref> --reveal-secrets` only for trusted development/test shells; it performs audited secret reveals for known project keys and materializes DB URL password placeholders.
...
go run ./cmd/supadupa-cli projects link --ref alpha --dir .
go run ./cmd/supadupa-cli projects env --ref alpha --out .supadupa/supabase.env
go run ./cmd/supadupa-cli projects env --ref alpha --reveal-secrets --out .env.supabase.local
go run ./cmd/supadupa-cli projects gen-types --ref alpha --out database.types.ts
...
go run ./cmd/supadupa-cli backups restore --ref alpha --backup-id <backup-id> --confirmation "restore project alpha"
go run ./cmd/supadupa-cli projects upgrade \
  --ref alpha \
  --version 15.8.1.085 \
  --backup-id <backup-id>
go run ./cmd/supadupa-cli branches create \
  ...
  --name "Alpha Preview" \
  --ttl-hours 24
# Add --with-data only when the branch should clone source project data.
